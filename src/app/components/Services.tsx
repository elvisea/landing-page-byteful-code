    ...
    >
      <div className="container mx-auto px-4">
        <div className={sectionHeader.wrapper}>
          <div className={sectionHeader.badge}>{t('services.badge')}</div>
          <h2 className={sectionHeader.title}>{t('services.title')}</h2>
          <p className={sectionHeader.subtitle}>{t('services.subtitle')}</p>
        </div>
          ...
          {items.map((item, index) => (
            <Card key={index} className={cardStyles.bordered}>
              <CardHeader className="pb-2">
                <div
                  className={`w-12 h-12 rounded-full ${bgColor.accentLight} flex items-center justify-center mb-4`}
                >
                  {getIcon(item.title)}
                </div>
                <CardTitle className={textColor.primary}>{item.title}</CardTitle>
                <CardDescription className={textColor.tertiary}>{item.subtitle}</CardDescription>
              </CardHeader>
              <CardContent>
                <p className={textColor.secondary}>{item.description}</p>
